...
- `CHATGPT_LOCAL_REQUEST_QUEUE`: `true|false` to serialize concurrent requests through FIFO queue (default true)
- `CHATGPT_LOCAL_AGGRESSIVE_MODE`: `true|false` to preprocess large HTML-like payloads (default false)
- `CHATGPT_LOCAL_UPSTREAM_TIMEOUT`: Timeout in seconds for upstream request (`0` = no timeout)
- `CHATGPT_LOCAL_HEADED`: Compatibility flag for headed-debug workflows
- `CHATGPT_LOCAL_TRAFFIC_LOG`: `true|false` to capture proxy traffic in memory (default true)
- `CHATGPT_LOCAL_TRAFFIC_MAX_ENTRIES`: Max retained traffic records in memory (default 300)
