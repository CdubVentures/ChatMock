...
def cmd_serve(
    ...
    if headed:
        eprint("INFO: --headed is enabled (compatibility flag). This proxy does not launch a browser in serve mode.")
    app = create_app(
        verbose=verbose,
        ...
        traffic_max_entries=traffic_max_entries,
    )
...
def main() -> None:
    ...
    p_serve.add_argument(
        "--aggressive-mode",
        action=argparse.BooleanOptionalAction,
        default=(os.getenv("CHATGPT_LOCAL_AGGRESSIVE_MODE") or "").strip().lower() in ("1", "true", "yes", "on"),
        help="Enable aggressive payload preprocessing for large HTML/DOM-like inputs.",
    )
    p_serve.add_argument(
        "--request-queue",
        action=argparse.BooleanOptionalAction,
        default=(os.getenv("CHATGPT_LOCAL_REQUEST_QUEUE", "true")).strip().lower() in ("1", "true", "yes", "on"),
        help="Serialize upstream requests through a FIFO queue to keep one active request at a time.",
    )
    p_serve.add_argument(
        "--upstream-timeout",
        type=float,
        default=_env_float("CHATGPT_LOCAL_UPSTREAM_TIMEOUT", 0.0),
        help="Upstream timeout in seconds. Use 0 for no timeout (infinite).",
    )
    p_serve.add_argument(
        "--headed",
        action=argparse.BooleanOptionalAction,
        default=(os.getenv("CHATGPT_LOCAL_HEADED") or "").strip().lower() in ("1", "true", "yes", "on"),
        help="Compatibility flag for headed browser debugging workflows.",
    )
    p_serve.add_argument(
        "--traffic-log",
        action=argparse.BooleanOptionalAction,
        default=(os.getenv("CHATGPT_LOCAL_TRAFFIC_LOG", "true")).strip().lower() in ("1", "true", "yes", "on"),
        help="Enable in-memory traffic monitor for all incoming proxy requests.",
    )
    p_serve.add_argument(
        "--traffic-max-entries",
        type=int,
        default=int(os.getenv("CHATGPT_LOCAL_TRAFFIC_MAX_ENTRIES", "300")),
        help="Maximum in-memory request records to keep in /debug/traffic.",
    )
    ...
    if args.command == "login":
        ...
        sys.exit(
            cmd_serve(
                ...
                request_queue_enabled=args.request_queue,
                upstream_timeout=(None if args.upstream_timeout <= 0 else args.upstream_timeout),
                headed=args.headed,
                traffic_log_enabled=args.traffic_log,
                traffic_max_entries=max(1, args.traffic_max_entries),
            )
        )
