...
## LLM Evaluation Lab (Docker Compose)

This repository now includes a standalone local evaluation lab:

- `chatmock` service (built from this local repository so proxy upgrades are included)
- `eval-bench` service (Node.js GUI at `http://localhost:4000`)

### Files

- `docker-compose.yml`: two-service setup plus optional login service
- `test-bench/`: custom Node.js API middleman + frontend split-view GUI

### One-time login (required)

ChatMock auth is stored in a persistent Docker volume named `chatmock_auth` mounted to `/data`, so you only need to do this once unless you delete the volume.
...
Alternative inside running container:
```bash
...
- If localhost callback fails in-browser, copy the full redirect URL back into the terminal prompt when asked.
...
- ChatMock OpenAI-compatible endpoint: `http://localhost:8000/v1`
- Eval Bench GUI: `http://localhost:4000`
...
- `Login-LLM-Eval-Lab.bat`

To stop everything, double-click:

- `Stop-LLM-Eval-Lab.bat`

### Sidecar provider config

Use these values in your main app:
...
Auth note: ChatMock tokens are persisted in the `chatmock_auth` Docker volume, so you usually do not need to login every launch.
...
- `chatmock` is started with `serve --expose-reasoning-models --aggressive-mode --request-queue --upstream-timeout 0`.
- Request queue mode serializes concurrent requests through a FIFO worker gate.
- Aggressive mode strips `script/style/svg` blocks from large HTML-like inputs before forwarding.
